"""/api/maintenance/* endpoints — auto-extracted.
...
@router .post ("/api/maintenance/cleanup-preview")
def cleanup_preview (req :RetentionRequest ):
    ...
    jobs =_app .db .jobs_older_than (days =req .days ,statuses =statuses )
    total_files_size =0 
    file_count =0 
    for j in jobs :
            ...
            Path (j .output_path ).with_suffix (".ppe_report.csv")):
                if path .is_file ():
                    ...
                    try :
                        ...
                        pass 
    deleted =_app .db .delete_jobs ([j .id for j in jobs ])
    return {
