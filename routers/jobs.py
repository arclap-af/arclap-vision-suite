...
@router.get("/api/jobs/{job_id}/_app.scan-thumb")
def job_scan_thumb(job_id: str):
    """Return the latest scanned-frame thumbnail for a filter _app.scan job, or 404
    ...
    if not db_path:
        raise HTTPException(404, "No _app.scan output yet")
...
def verify_job(job_id: str, req: _app.VerifyRequest):
...
def stop_job(job_id: str):
    ...
    if _app.runner.is_running() == job_id:
        ...
        # Best-effort: leave it in _app.queue but mark stopped so worker skips
