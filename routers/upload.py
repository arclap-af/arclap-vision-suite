"""/api/upload/* endpoints — auto-extracted.
...
async def upload (file :UploadFile =File (...)):
    ...
    if suffix not in _app .ALLOWED_VIDEO_EXTS :
        raise HTTPException (
        415 ,
        f"Unsupported file type '{suffix }'. Allowed: {', '.join (sorted (_app .ALLOWED_VIDEO_EXTS ))}",
        )

    file_id =uuid .uuid4 ().hex [:12 ]
    dest =_app .UPLOADS /f"{file_id }{suffix }"
    ...
    with open (dest ,"wb")as f :
        while chunk :=await file .read (1 <<20 ):
            ...
            if written >_app .MAX_UPLOAD_BYTES :
                ...
                raise HTTPException (
                413 ,
                f"File exceeds {_app .MAX_UPLOAD_BYTES //(1024 **3 )} GB upload limit.",
                )
            f .write (chunk )
